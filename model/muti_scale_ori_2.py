...
class MSResNet(nn.Module):
    def __init__(self, input_channel, layers=[1, 1, 1, 1], num_classes=2, include_top = False,include_top_kan = True):
        self.inplanes3 = 64
        self.inplanes5 = 64
        self.inplanes7 = 64
        self.include_top = include_top
        self.include_top_kan = include_top_kan   #添加kan层---------------------------------------------------------
        ...
        if self.include_top:  #-------------------------------------------------------------------------------------
            self.self.fc = nn.Linear(28416,num_classes)
        if self.include_top_kan:
            self.kan = KAN([28416,128,num_classes])
